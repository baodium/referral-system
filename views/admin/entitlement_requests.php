...
<td> N<?php $amt=$this->users_model->get_acategory($user_info['amount_category']); echo $amt['entitlement']*(floor($user_info['entitlement_count']/5)) ?></td>
<td><?php echo $page['account_number'] ?></td>
<td><?php echo $page['account_name'] ?></td>
<td><?php echo ($page['product_category']=="")?"<center>-</center>":$page['product_category'] ?></td>
<td><?php echo '<span style="color:red">'.$page['description'].'</span>'; ?></td>
<td><?php echo ($page['status']=="rejected")?"<span style='color:green'>Rejected</span>":"<span style='color:orange'>Pending</span>"; ?></td>
<td><?php $date=explode(" ",$page['date']);echo $date[0] ?></td>
<td><a href="<?php echo $this->config->config['base_url']?>myadmin/process_entitlement/<?php echo $page['request_id'] ?>" style="margin-top:10px" class="btn btn-primary btn-success">Process </a><br/><br/></td>
<td><input type="submit" name="delete" style="margin-top:10px" onclick="return checkReject('user_form');" class="btn btn-primary btn-danger" value="Reject" /><br/><br/></td>
...
?>
<tr><td colspan="12"><center><br/>No request<br/></center></td></tr>
<?php } ?>
</table><br/><br/>
<?php
